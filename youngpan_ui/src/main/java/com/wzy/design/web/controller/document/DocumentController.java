package com.wzy.design.web.controller.document;
...
@Controller
@Scope("prototype")
@RequestMapping("/operator/document")
public class DocumentController extends BaseController {
	
	private static final String JSP_PRIX = "operator/document/";
	...
	public ModelAndView index(){
		Long docmentNum = fileInfoService.queryAllDocument(getCurrentUserName());
    	OperatorModelAndView omv = new OperatorModelAndView(docmentNum > 0 ?INDEX:UPLOAD);
    ...
    public JQGridPage<?> list(int page, int rows) {
		...
		for(Document ocument:Document.values()){
			documentTypes.add(ocument.getLabel());
		}
        Page<FileInfo> pager = fileInfoService.queryFileByContentType(getCurrentUserName(),documentTypes, (page - 1) * rows, rows);
